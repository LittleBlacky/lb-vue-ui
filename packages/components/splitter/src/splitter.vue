...
watch([panelInstances, splitterSize], () => {
  ...
  if (total > splitterSize.value || !emptyPanels.length) {
    panelInstances.value.forEach((panel) => {
      panel.sizeRef.value =
        ((panel.sizeRef.value ?? 0) / total) * splitterSize.value;
    });
  } else {
...
const unregisterPanel = (index: number) => {
  ...
  for (let i = index; i < panelInstances.value.length; ++i)
    --panelInstances.value[i].index;
};

const panelTotalSize = computed(() => {
  return panelInstances.value.reduce(
    (acc, cur) => acc + (cur.sizeRef.value ?? 0),
    0
  );
});
