...
export const LbResolver = (
  options: LbsolverOptions = {}
): ComponentResolverFunction => {
  const {
    autoImportStyle = true,
    prefix = "Lb", // 组件前缀
    libraryName = "@littleblacky/lb-vue-ui", // 组件库名称
    stylePath = `${libraryName}/style`, //样式文件路径
  } = options;
  return (name: string) => {
    ...
    if (autoImportStyle && stylePath) {
      const styleExt = autoImportStyle
        ? "css"
        : autoImportStyle === "sass"
          ? "scss"
          : "css";

      result.sideEffects = [`${stylePath}/${tagName}.${styleExt}`];
    }
